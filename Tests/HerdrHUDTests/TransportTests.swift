...
final class TransportTests: XCTestCase {
    ...
    func testLiteralRemoteArguments() throws {
        ...
        let inv = try client.invocation(remote,["agent","prompt","w1:p1",prompt],mutation:true)
        XCTAssertEqual(inv.executable,"/usr/bin/ssh"); XCTAssertTrue(inv.arguments.contains("StrictHostKeyChecking=yes"))
        XCTAssertTrue(inv.arguments.last!.hasSuffix(shellQuote(prompt)))
        XCTAssertTrue(inv.arguments.last!.contains(shellQuote("dev")))
    }
    ...
    func clientWith(_ state: @escaping () -> Row, sends: @escaping (Invocation) throws -> String = { _ in "{\"result\":{\"ok\":true}}" }) -> HerdrClient {
    ...
    func testReadyPromptOnceAndLiteral() throws {
        var sent:[Invocation]=[];let client = clientWith({self.row()},sends:{ sent.append($0); return "{\"result\":{}}" })
        let snapshot=client.snapshot();let id=(snapshot["agents"] as! [Row])[0]["id"] as! String
        _ = try client.prompt(id,"literal $(printf secret)\nsecond line")
        XCTAssertEqual(sent.count,1);XCTAssertEqual(sent[0].arguments.last,"literal $(printf secret)\nsecond line")
    }
